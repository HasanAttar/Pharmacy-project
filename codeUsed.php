...
foreach($data as $row) {
    if($count > 0) {
        ...
        $usedQuery = "INSERT INTO usedmedicines (Name, Expirationdate, Quantity, Price, Discount,total_price, pharmacy_name, region,pharmacist_mobile) VALUES ('$Name', '$expired', '$quantity','$price', '$discount','$total_price', '$pharmacy_name', '$region','$mobile')";
        $result = mysqli_query($con, $usedQuery);
        $msg = true;
    } else {
        $count = 1;
    }
        ...
        {
            $_SESSION['message'] = "";
            header('Location: fetchused.php?pharmacy_name='.$pharmacy_name.'&region='.$region.'&mobile='.$mobile);
            exit(0);
        }
        else
        {
            $_SESSION['message'] = "Not Imported";
            header('Location: codeused.php');
            exit(0);
  }
...
<div class="card" style= "height:100%;font-size:25px; text-align:center;position:absolute;left:50%;transform: translateX(-50%); ">
                    <div class="card-header">
                        <h4 style="font-size:25px">Import Used medicines list  </h4>
                    </div>
                    <div class="card-body">

                    <form method="post" action="codeused.php" enctype="multipart/form-data">
